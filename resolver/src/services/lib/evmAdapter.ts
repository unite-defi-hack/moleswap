...
export class EvmAdapter {
  ...
  private async getSrcDeployEvent(
    ...
    if (logs.length === 0) {
      throw new Error("No logs found for SrcEscrowCreated event");
    }
    const log = logs[0];
    if (!log) {
      throw new Error("No log data found for SrcEscrowCreated event");
    }
  ...
  /**
   * Streamlined method to create destination escrow using Order from DB
   */
  async createDestinationEscrowFromOrder(
    ...
    const hashLock = order.makerTraits; // hashlock is stored in makerTraits
    const maker = BigInt(order.receiver);
    const token = BigInt(order.takerAsset);
  ...
  /**
   * Streamlined method to withdraw from destination escrow using Order from DB
   */
